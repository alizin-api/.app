...
const limitamo = "50"
document.getElementById("limit").textContent=limitamo

// Visitor
$.getJSON("https://api.countapi.xyz/hit/api.smkxt7.my.id/visitor", function(response) {
    $("#visitors").text(response.value);
})

// User Login
$.getJSON("https://api.countapi.xyz/hit/api.smkxt7.my.id/userlogin", function(response) {
    $("#userlogin").text(response.value);
})

// Request Today
$.getJSON("https://api.countapi.xyz/hit/api.smkxt7.my.id/requesttoday", function(response) {
    $("#requesttoday").text(response.value);
})

// Total Request
$.getJSON("https://api.countapi.xyz/hit/d/totalrequest", function(response) {
...
myDays = ['Semana','Segunda-feira','Segunda-feira','Quarta-feira','Quinta-feira','Sexta-feira','Sábado'];
var tgl = new Date();
var day = tgl.getDate()
bulan = tgl.getMonth()
...
// Count Time
var countDownDate = new Date("julho dia 10, 2022 00:00:00").getTime();
...
// Saying Time
if(Jam<4){saying.innerHTML="bons primeiros dias";}else
if(jam<11){saying.innerHTML="Bom Dia";}else 
if(Jam<16){saying.innerHTML="Boa tarde";}else 
if(jam<20){saying.innerHTML="Boa tarde";}else
